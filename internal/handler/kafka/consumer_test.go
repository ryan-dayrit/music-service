...
// TestConsume_ConsumeRetry tests the retry logic when Consume returns non-fatal errors
func TestConsume_ConsumeRetry(t *testing.T) {
	mockCG := new(MockConsumerGroup)
	...
	callCount := 0
	// Mock the Consume method to return nil first time, then context canceled
	mockCG.On("Consume", mock.Anything, []string{"test-topic"}, mock.Anything).Run(func(args mock.Arguments) {
		callCount++
		ctx := args.Get(0).(context.Context)
		handler := args.Get(2).(sarama.ConsumerGroupHandler)
		_ = handler.Setup(nil)
		if callCount == 1 {
			// First call: return immediately to trigger retry
			return
		}
		// Second call: wait for context cancellation
		<-ctx.Done()
	}).Return(nil)
	...
	assert.GreaterOrEqual(t, callCount, 1, "Consume should be called at least once")
	mockCG.AssertExpectations(t)
}
